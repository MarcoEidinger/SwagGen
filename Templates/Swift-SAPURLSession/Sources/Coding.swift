...
{{ options.apiAccessLevel }} protocol {{ options.modelProtocol }}: Codable, Equatable { }
{% endif %}

{% for type, typealias in options.typeAliases %}
{{ options.apiAccessLevel }} typealias {{ type }} = {{ typealias }}
{% endfor %}

{{ options.apiAccessLevel }} protocol ResponseDecoder {
...
{{ options.apiAccessLevel }} protocol RequestEncoder {
...
extension KeyedDecodingContainer {
    ...
    {{ options.apiAccessLevel }} func decodeArray<T: Decodable>(_ key: K) throws -> [T] {
    ...
    {{ options.apiAccessLevel }} func decodeArrayIfPresent<T: Decodable>(_ key: K) throws -> [T]? {
        return try decodeOptional {
            if contains(key) {
...
{{ options.apiAccessLevel }} struct DateDay: Codable, Comparable {

    /// The date formatter used for encoding and decoding
    {{ options.apiAccessLevel }} static let dateFormatter: DateFormatter = {
    ...
    {{ options.apiAccessLevel }} let date: Date
    {{ options.apiAccessLevel }} let year: Int
    {{ options.apiAccessLevel }} let month: Int
    {{ options.apiAccessLevel }} let day: Int

    {{ options.apiAccessLevel }} init(date: Date = Date()) {
        self.date = date
    ...
    {{ options.apiAccessLevel }} init(year: Int, month: Int, day: Int) {
    ...
    {{ options.apiAccessLevel }} init(from decoder: Decoder) throws {
        let container = try decoder.singleValueContainer()
        let string = try container.decode(String.self)
    ...
    {{ options.apiAccessLevel }} func encode(to encoder: Encoder) throws {
        var container = encoder.singleValueContainer()
        let string = DateDay.dateFormatter.string(from: date)
        try container.encode(string)
    }

    {{ options.apiAccessLevel }} static func == (lhs: DateDay, rhs: DateDay) -> Bool {
...
extension DateFormatter {
    ...
    {{ options.apiAccessLevel }} func string(from dateDay: DateDay) -> String {
